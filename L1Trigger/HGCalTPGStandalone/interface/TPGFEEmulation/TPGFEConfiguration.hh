...
namespace TPGFEConfiguration{
  ...
  public:
    ...
    void setDropLSB(uint32_t dLSB) { assert(dLSB<=4); dropLSB = dLSB;} //as uint32_t dLSB>=0
    void setSelect(uint32_t sel) { assert(sel==1 or sel==2); select = sel;}
    void setSTCType(uint32_t stctype) { assert(stctype<=4); stc_type = stctype;} //as uint32_t stctype>=0
