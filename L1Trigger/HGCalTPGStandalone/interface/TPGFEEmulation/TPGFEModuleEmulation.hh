...
namespace TPGFEModuleEmulation{
  ...
  void ECONTEmulation::Emulate(bool isSim, uint64_t ievent, uint32_t& moduleId, const std::map<uint32_t,TPGFEDataformat::ModuleTcData>& moddata){
    ...
    if(outputType==TPGFEDataformat::BestC)
      ...
      std::cerr << "ECONTEmulation::Emulate: Unexpected ECON-T output type (expected list BestC,STC)" << std::endl ;
  }
  ...
  void ECONTEmulation::EmulateSTC(bool isSim, uint64_t ievent, uint32_t& moduleId, const std::map<uint32_t,TPGFEDataformat::ModuleTcData>& moddata) {
	...
	for(const auto& econtc : tclist){
	  ...
	  if(!hasFound){
	    std::cerr << "ECONTEmulation::EmulateSTC (moduleid="<<moduleId<<") : Mux not set for TC " << econtc << std::endl;
	    continue;
	  }
	  ...
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateSTC4 (moduleid="<<moduleId<<", TC="<<econtc<<", hgctc="<<hgctc<<") decompressed: " << decompressed << ", decompressed*calib: " << decomp64bit << std::endl; 
	  decompressed =  decomp64bit >> 11;
	  decompressedSTC += decompressed  ;
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateSTC4 (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed*calib>>11: " << decompressed << ", decompressedSTC: " << decompressedSTC << std::endl; 
	...
	if(isVerbose) std::cout << "ECONTEmulation::EmulateSTC4 (moduleid="<<moduleId<<", STC="<< istc <<") tcrawdatalist.size: " << tcrawdatalist.size() << std::endl;
	...
	if(isVerbose) std::cout << "ECONTEmulation::EmulateSTC4 Before sorting....(moduleid="<<moduleId<<", STC="<< istc <<"), modified size: " << tcrawdatalist.size() << std::endl;
	if(isVerbose) for(uint32_t i=0;i<tcrawdatalist.size();i++) tcrawdatalist[i].print();
	std::sort(tcrawdatalist.begin(),tcrawdatalist.end(),TPGFEDataformat::TcRawDataPacket::customLTA);
	if(isVerbose) std::cout << "ECONTEmulation::EmulateSTC4 After sorting....(moduleid="<<moduleId<<", STC="<< istc <<"), modified size: " << tcrawdatalist.size() << std::endl;
	if(isVerbose) for(uint32_t i=0;i<tcrawdatalist.size();i++) tcrawdatalist[i].print();
	...
	for(const auto& econtc : tclist){
	  ...
	  if(!hasFound){
	    std::cerr << "ECONTEmulation::EmulateBC (moduleid="<<moduleId<<") : Mux not set for TC " << econtc << std::endl;
	    continue;
	  }
	  if(mdata.getTC(emultc).isTcTp1()) isTcTp1 = true;
	  if(mdata.getTC(emultc).isTcTp2()) isTcTp2 = true;
	  ...
	  uint64_t decompressed = DecompressEcont(mdata.getTC(emultc).getCdata(), configs.getEconTPara().at(moduleId).getDensity());
	  uint64_t decomp64bit = decompressed * configs.getEconTPara().at(moduleId).getCalibration(econtc) ;
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateSTC16 (moduleid="<<moduleId<<", TC="<<econtc<<", hgctc="<<hgctc<<") decompressed: " << decompressed << ", decompressed*calib: " << decomp64bit << std::endl; 
	  decompressed =  decomp64bit >> 11;
	  decompressedSTC16 += decompressed ;
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateSTC16 (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed*calib>>11: " << decompressed << ", decompressedSTC16: " << decompressedSTC16 << std::endl; 
	  uint16_t compressed_bc = CompressEcontBc(decompressed,dropLSB);
	  tcdata.setTriggerCell(TPGFEDataformat::BestC, econtc, compressed_bc, decompressed, mdata.getTC(emultc).isTcTp1(), mdata.getTC(emultc).isTcTp2(), mdata.getTC(emultc).isTcTp3()) ;
	  tcrawdatalist.push_back(tcdata);
	}
  ...
  void ECONTEmulation::EmulateJulSTC(bool isSim, uint64_t ievent, uint32_t& moduleId, const std::map<uint32_t,TPGFEDataformat::ModuleTcData>& moddata) {
	...
	for(const auto& econtc : tclist){
	  ...
	  if(!hasFound){
	    std::cerr << "ECONTEmulation::EmulateJulSTC (moduleid="<<moduleId<<") : Mux not set for TC " << econtc << std::endl;
	    continue;
	  }
	  if(mdata.getTC(emultc).isTcTp1()) isTcTp1 = true;
	  ...
	  uint64_t decompressed = DecompressEcont(mdata.getTC(emultc).getCdata(),configs.getEconTPara().at(moduleId).getDensity());
	  uint64_t decomp64bit =  decompressed * configs.getEconTPara().at(moduleId).getCalibration(econtc) ;
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateJulSTC4 (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed: " << decompressed << ", decompressed*calib: " << decomp64bit << std::endl; 
	  decompressed =  decomp64bit >> 11;
	  decompressedSTC += decompressed ;
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateJulSTC4 (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed*calib>>11: " << decompressed << ", decompressedSTC: " << decompressedSTC << std::endl; 
	  uint16_t compressed_bc = CompressEcontBc(decompressed, dropLSB);
	  tcdata.setTriggerCell(TPGFEDataformat::BestC, econtc, compressed_bc, decompressed, mdata.getTC(emultc).isTcTp1(), mdata.getTC(emultc).isTcTp2(), mdata.getTC(emultc).isTcTp3()) ;
	  tcrawdatalist.push_back(tcdata);
	}
	...
	uint32_t lMaxId = findLastMax(tcrawdatalist);
	tcrawdatalist.resize(lMaxId+1);
	if(isVerbose) std::cout << "ECONTEmulation::EmulateJulSTC4 (moduleid="<<moduleId<<", STC="<< istc <<") size: " << tcrawdatalist.size() << std::endl;
	if(isVerbose) for(uint32_t i=0;i<tcrawdatalist.size();i++) tcrawdatalist[i].print();
	...
	for(const auto& econtc : tclist){
	  ...
	  if(!hasFound){
	    std::cerr << "ECONTEmulation::EmulateJulSTC16 (moduleid="<<moduleId<<") : Mux not set for TC " << econtc << std::endl;
	    continue;
	  }
	  if(mdata.getTC(emultc).isTcTp1()) isTcTp1 = true;
	  if(mdata.getTC(emultc).isTcTp2()) isTcTp2 = true;
	  ...
	  uint64_t decompressed = DecompressEcont(mdata.getTC(emultc).getCdata(), configs.getEconTPara().at(moduleId).getDensity());
	  uint64_t decomp64bit = decompressed * configs.getEconTPara().at(moduleId).getCalibration(econtc) ;
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateJulSTC16 (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed: " << decompressed << ", decompressed*calib: " << decomp64bit << std::endl; 
	  decompressed =  decomp64bit >> 11;
	  decompressedSTC16 += decompressed ;
	  if(isVerbose) std::cout << "ECONTEmulation::EmulateJulSTC16 (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed*calib>>11: " << decompressed << ", decompressedSTC16: " << decompressedSTC16 << std::endl; 
	  uint16_t compressed_bc = CompressEcontBc(decompressed,dropLSB);
	  tcdata.setTriggerCell(TPGFEDataformat::BestC, econtc, compressed_bc, decompressed, mdata.getTC(emultc).isTcTp1(), mdata.getTC(emultc).isTcTp2(), mdata.getTC(emultc).isTcTp3()) ;
	  tcrawdatalist.push_back(tcdata);
	}
  ...
  void ECONTEmulation::EmulateBC(bool isSim, uint64_t ievent, uint32_t& moduleId, const std::map<uint32_t,TPGFEDataformat::ModuleTcData>& moddata) {
	...
	continue;
      ...
      if(isVerbose) std::cout << "ECONTEmulation::EmulateBC (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed: " << decompressed << ", decompressed*calib: " << decomp64bit << std::endl; 
      decompressed =  decomp64bit >> 11;
      decompressedMS += (decompressed >> dropLSB) ;
      if(isVerbose) std::cout << "ECONTEmulation::EmulateBC (moduleid="<<moduleId<<", TC="<<econtc<<") decompressed*calib>>11: " << decompressed << ", decompressedMS: " << decompressedMS << std::endl; 
      uint16_t compressed_bc = CompressEcontBc(decompressed,dropLSB);
